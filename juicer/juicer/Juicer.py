    ...
    def delete_rpms(self, repo_name, rpms, env):
        ...
        for rpm in rpms:
            ...
            if _r.status_code != Constants.PULP_POST_ACCEPTED:
                ...
                unassociate_task = juicer.utils.load_json_str(_r.content)['spawned_tasks'][0]['task_id']
                ...
                juicer.utils.Log.log_info("Remove call for %s from %s was accepted" %
                                          (rpm, repo_name))
                _r = self.connectors[env].delete('/content/orphans/')
                delete_task = juicer.utils.load_json_str(_r.content)['spawned_tasks'][0]['task_id']
